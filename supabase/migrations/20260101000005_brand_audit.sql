...
create policy "equipment_logs_select" on equipment_logs for select using (auth.uid() = user_id);
drop policy if exists "equipment_logs_insert" on equipment_logs;
create policy "equipment_logs_insert" on equipment_logs for insert with check (auth.uid() = user_id);
