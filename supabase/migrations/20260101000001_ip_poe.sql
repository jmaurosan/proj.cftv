...
create unique index if not exists uq_cameras_dvr_channel on cameras(dvr_id, channel_number)
  where dvr_id is not null;

-- Para câmeras IP: ip_address deve ser único por usuário
create unique index if not exists uq_cameras_ip_user on cameras(user_id, ip_address)
