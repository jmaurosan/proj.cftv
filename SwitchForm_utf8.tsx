...
export default function SwitchForm({ initialData, onSubmit, onCancel }: SwitchFormProps) {
  ...
  const handleModelSelect = (modelId: string) => {
    ...
    // N├úo preenche name - ├® o identificador do Switch espec├¡fico
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input label="Nome" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Ex: Switch Sala T├®cnica" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input label="Localiza├º├úo" value={location} onChange={(e) => setLocation(e.target.value)} required placeholder="Ex: Rack Sala T├®cnica" />
      ...
      <Input label="Observa├º├Áes" value={notes} onChange={(e) => setNotes(e.target.value)} />

      {/* Se├º├úo de Portas do Switch */}
      {switchId && (
        <div className="border border-slate-700 rounded-lg p-4 space-y-3">
          <h3 className="text-sm font-semibold text-primary flex items-center gap-2">
            <Plug className="w-4 h-4" />
            Conex├Áes das Portas
          </h3>
