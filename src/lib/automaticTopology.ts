export type AutomaticTopologyNodeType = 'internet' | 'router' | 'switch' | 'dvr' | 'camera' | 'rack' | 'balun' | 'monitor'
...
export function buildAutomaticTopologyConnections({
  ...
  const add = (source: string, target: string, label?: string, idSuffix = `${source}-${target}`) => {
    ...
    connections.push({ id: `auto-${idSuffix}`, source, target, active: isActive(targetNode.status), label, style: 'dashed' })
  }

  routers.forEach((router) => add('internet', router.id, 'Link', `internet-${router.id}`))
  ...
  switchPorts
    ...
    .forEach((port) => {
      ...
      if (device.type === 'router') add(device.id, port.switch_id, `P${port.port_number}`, `switch-port-${port.switch_id}-${port.port_number}`)
      else add(port.switch_id, device.id, `P${port.port_number}`, `switch-port-${port.switch_id}-${port.port_number}`)
    })

  switches.forEach((networkSwitch) => {
    if (!connectedTargets.has(networkSwitch.id)) {
      if (routers[0]) add(routers[0].id, networkSwitch.id, 'LAN', `router-switch-${networkSwitch.id}`)
      else add('internet', networkSwitch.id, 'Link', `internet-switch-${networkSwitch.id}`)
    }
  })

  dvrs.forEach((dvr) => {
    if (!connectedTargets.has(dvr.id) && switches[0]) add(switches[0].id, dvr.id, 'LAN', `switch-dvr-${dvr.id}`)
  })
  ...
  cameras.forEach((camera) => {
    ...
    if (isIp && switchNode?.type === 'switch') {
      add(camera.switch_id!, camera.id, camera.switch_port ? `P${camera.switch_port}` : 'IP/PoE', `switch-camera-${camera.id}`)
      return
    }

    if (balunId && nodeById.get(balunId)?.type === 'balun') {
      if (camera.dvr_id && nodeById.get(camera.dvr_id)?.type === 'dvr') {
        add(camera.dvr_id, balunId, 'Vídeo', `dvr-balun-${camera.dvr_id}-${balunId}`)
      }
      add(balunId, camera.id, camera.balun_port || balunPort?.port_number ? `P${camera.balun_port || balunPort?.port_number}` : 'UTP', `balun-camera-${camera.id}`)
    ...
    if (camera.dvr_id && nodeById.get(camera.dvr_id)?.type === 'dvr') {
      add(camera.dvr_id, camera.id, camera.channel_number ? `CH${camera.channel_number}` : 'Coaxial', `dvr-camera-${camera.id}`)
      const created = connections.at(-1)
      if (created?.target === camera.id) created.active = cameraActive
    }
  })
